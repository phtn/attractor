    ...
    const init = async () => {
      ...
      const vertexShaderCode = `
        ...
        fn main(@builtin(vertex_index) index: u32) -> VertexOut {
          ...
          // Pass pixel coordinates (0 to resolution)
          out.fragCoord = (pos[index] * 0.5 + 0.5) * vec2<f32>(${canvasWidth}.0, ${canvasHeight}.0);
          return out;
        }
      `;
      ...
      const fragmentShaderCode = `
        ...
        fn main(@location(0) fragCoord: vec2<f32>) -> @location(0) vec4<f32> {
          ...
          for (var i = 1.0; i <= 100.0; i = i + 1.0) {
            ...
            // Fixed: proper vec4 construction
            let cosP = 0.1 * cos(p * 3.0) - 0.1;
            d = 0.2 * length(vec4<f32>(cosP, p.z));
            z = z + d;
            // Fixed: simplified the complex arithmetic
            let phase = i * 0.7 + t + 6.0;
            o = o + (1.0 + cos(phase)) / (d * i);
          }
          o = tanh(o * o / 900.0);
